...
class AppsDashboardController {
	...
	async update(req, res, next) {
		...
		//Save new field 'img'
		if (imgObj) obj.img = `/upload/app/${req.body.name}/${imgObj[0].filename}`
		else obj.img = req.body.oldImg

		Apps.findOne({ _id: req.params.id })
			.then(data => {
				if (!data) return Promise.reject()
				if (data.name !== obj.name) renameFolder(data.name, obj.name, 'src/public/upload/app')
			})
			.catch(next)
	...
	recylePage(req, res, next) {
		...
		Promise.all([
			Apps.find({ deletedAt: { $gte: new Date(1970, 0, 1) } })
				.sortable(req, res)
				.paginationHandle(req, res),
			Apps.countDocuments({ deletedAt: { $gte: new Date(1970, 0, 1) } }),
			Apps.find({ deletedAt: { $gte: new Date(1970, 0, 1) }, name: regex })
				.sortable(req, res)
				.paginationHandle(req, res),
	...
	deleteForever(req, res, next) {
		Promise.all([Apps.findOne({ _id: req.params.id }), Apps.deleteOne({ _id: req.params.id })])
			.then(([data]) => {
				if (!data) return Promise.reject()
